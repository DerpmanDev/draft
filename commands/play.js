...
module.exports = {
    ...
    run: async (client, message, args, { GuildDB }) => {
        if (!message.member.voice.channel) return client.sendTime(message.channel, "❌ | **play some songs. it use spotify search**");
        if (message.guild.me.voice.channel && message.member.voice.channel.id !== message.guild.me.voice.channel.id) return client.sendTime(message.channel, ":x:|**You need to be in the same voice channel as me to use the command idiot**");
       ...
       return client.sendTime(message.channel,"❌ | **Lavalink node not connected**");
        ...
        if (!player) return client.sendTime(message.channel, "❌ | **Nothing is running...**");
        ...
        try {
            if (SearchString.match(client.Lavasfy.spotifyPattern)) {
                ...
                if (Searched.loadType === "PLAYLIST_LOADED") {
                    ...
                    SongAddedEmbed.setAuthor(`Add to queue`, client.config.IconURL);
                    SongAddedEmbed.setDescription(`[${Searched.tracks[0].info.title}](${Searched.tracks[0].info.uri})`);
                    SongAddedEmbed.addField("Artist", Searched.tracks[0].info.author, true);
                    ...
                    return client.sendTime(message.channel, "**Not Found - **" + SearchString);
                }
            } else {
                let Searched = await player.search(SearchString, message.author);
                if (!player) return client.sendTime(message.channel, "❌ | **Nothing is playing...**");

                if (Searched.loadType === "NO_MATCHES") return client.sendTime(message.channel, "**Not found - **" + SearchString);
    ...
    SlashCommand: {
        ...
        run: async (client, interaction, args, { GuildDB }) => {
            ...
            if (!member.voice.channel) return client.sendTime(interaction, "❌ | **You need to be in a vc to use this idiot**");
            if (guild.me.voice.channel && !guild.me.voice.channel.equals(member.voice.channel)) return client.sendTime(interaction, ":x: | **You need to be in a vc to use this idiot**");
            let CheckNode = client.Manager.nodes.get(client.config.Lavalink.id);
            if (!CheckNode || !CheckNode.connected) {
            ...
            if (search.match(client.Lavasfy.spotifyPattern)) {
                ...
                try {
                    ...
                    if (res.loadType === "LOAD_FAILED") {
                        if (!player.queue.current) player.destroy();
                        return client.sendError(interaction, `:x: | **a error happened while searching. oops**`);
                    }
                } catch (err) {
